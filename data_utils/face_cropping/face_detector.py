...
class FaceDetector:
    ...
    def get_instance(model_path):
        ...
        if FaceDetector.__instance is None:
            FaceDetector(model_path)
        return FaceDetector.__instance
    
    def __init__(self, model_file=None, session=None):
        print('Initialize Face Detector ... ')
        if FaceDetector.__instance is not None:
    ...
    def detect_image(self, image):
        bboxes, kpss = self.detect(image)
        if len(bboxes)==0:
            return [], []
        else:
            return bboxes[0], kpss[0].reshape(10)
    ...
    for sub_dir in sub_dirs:
        if sub_dir not in os.listdir(save_dir):
            ...
            for name in tqdm(names):
                img_path = os.path.join(data_dir, sub_dir, name)
                ...
                for i in range(bboxes.shape[0]):
                    ...
                    if face_img.shape[0] > 40 and face_img.shape[1] > 40:
                        face_dir = os.path.join(save_dir, sub_dir)
                        Path(face_dir).mkdir(exist_ok=True, parents=True)
                        cv2.imwrite(os.path.join(face_dir, name[:-4] + f'_{i}.jpg'), face_img)
